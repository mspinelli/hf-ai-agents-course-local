...
# Setup telemetry traces for the agent. The code in this modules will run
# inside of docker and assumes the telemtry server (phoenix) is running on
# the host machine.
#
...
# has been downloaded on the host machine already. As with the telemtry server,
...
# it must be installed on the host machine.
model = LiteLLMModel(
...
# If we weren't using telementry then this becomes more important, but the
# output of the response will show in a more structured way in on the host
# machine via the telemtry daashboard at http://127.0.0.1:6006/projects by
# default
print(response)
